...
def get_roadmap(goal_id: int, db: Session = Depends(get_db)):
    items = (
        db.query(RoadmapItem)
        .filter(RoadmapItem.goal_id == goal_id)
        .order_by(RoadmapItem.id.asc())
        .all()
    )
    return {
        "goal_id": goal_id,
...
def reschedule_goal(goal_id: int, db: Session = Depends(get_db)):
    ...
    reschedule_roadmap(items, goal.exam_date)
    db.commit()
...
def delete_goal(goal_id: int, db: Session = Depends(get_db)):
    ...
    if goal.syllabus_file:
        try:
            file_path = Path(goal.syllabus_file)
            if file_path.exists():
                file_path.unlink()
        except Exception:
            pass
